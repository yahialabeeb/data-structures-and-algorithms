...
function transformToLis(obj) {
  ...
  arr1.forEach((val, idx) => {
    results.push(`<li>${val}: ${arr2[idx]}</li>`)
  })
  return results
}
...
const count = (target, input) => {
  ...
  for (let i = 0; i < input.length; i++) {
    let midRsult = input[i].reduce((rep2, value) => {
      rep2 = (value === target) ? rep2 + 1 : rep2
      return rep2
    }, 0)
    result = result + midRsult
  }
  return result
};
...
const divisibleByFiveTwoToThePower = (input) => {
  ...
  for (let i = 0; i < input.length; i++) {
    result[i] = [];
    let rere = input[i].filter(ele => typeof ele !== "string")
    let midRsult = rere.filter(num => num % 5 === 0)
    if (midRsult) {
      for (let q = 0; q < midRsult.length; q++) {
        result[i].push(Math.pow(2, midRsult[q]))
      }
    }
  }
  return result
...
let findMaleAndFemale = (data) => {
  ...
  for (let i = 0; i < data.length; i++) {
    if (data[i].gender === 'female' || data[i].gender === 'male') {
      resultarr[i] = (count === 0) ? `${data[i].name}` : ` and ${data[i].name}`
      count = 1
    }
    else {
      resultarr[i] = ''
...
let findShortest = (data) => {
  let hi = ""
  let result = data.reduce((acc, obj, index) => {
    if (Number(obj.height) <= acc) {
      acc = Number(obj.height)
    }
    return acc
  }, 200)
  data.forEach(obj => {
     hi = (obj.height == result) ? obj.name: hi
  })
