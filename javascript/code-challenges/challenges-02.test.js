...
const raisedToTheThird = (arr) => {
  let arrofcube = []
  arr.forEach((val) => {
    let valcu = Math.pow(val, 3)
    arrofcube.push(valcu)
  }
  )
  return arrofcube
};
...
const forEachTwoToThe = (arr) => {
  let arrOfResult = []
  arr.forEach((val) => {
    arrOfResult.push(Math.pow(2, val))
  })
  return arrOfResult
};
