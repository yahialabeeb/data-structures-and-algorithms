...
const screenForNames = (arr) => {
  let reg = /(Mr.|Mrs.|Ms.|Dr.) [a-zA-Z]/gm
  let result = []
...
const toTitleCase = (arr) => {
  ...
  arr.forEach(str => {
    stt = str.match(reg)
    if (stt) {
      stt = stt[0].toUpperCase()
      result.push(str.replace(reg, stt))
    } else
      result.push(str)
  })
  return result
...
let biggerThanLuke = (arr) => {
  ...
  let big = []
  arr.forEach((obj) => {
    if (Number(obj.mass) > Number(arr[0].mass)) {
      big.push(obj.name)
    }
  })
  big.forEach((ele, idx) => {
...
const detectTicTacToeWin = (board) => {
  let result = false
  for (let i = 0; i < 3; i++) {
    if (board[i][0] === board[i][1] && board[i][1] === board[i][2] && board[1][2]) {
      result = true
    }
  }
  for (let i = 0; i < 3; i++) {
